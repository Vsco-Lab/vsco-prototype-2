...
이를 통해 RAG 문서는 시장 구조·기업 전략의 **기초 지식 베이스** 역할을, 웹 검색은 **실행 시점의 최신 동향** 역할을 분담하며, 보고서를 생성할 때마다 Serper.dev를 통한 실시간 Google 검색 결과가 반영된다.
...
Tavily와 Serper.dev에 대한 벤치마크를 실시하여 웹 검색 도구를 선정하였다.
...
> Tavily가 응답 속도와 결과 풍부함에서 우수했으나, 본 프로젝트에서는 **Serper.dev**를 선정하였다. Agent당 3~4회 웹 검색이 실행되며 1회 보고서 생성에 약 15~20회의 검색이 소비되므로, Tavily의 무료 한도(1,000회)로는 약 50~66회 실행이 한계인 반면, Serper.dev(2,500회)는 약 125~166회 실행이 가능하여 **경제성 측면에서 2.5배 유리**하다.
